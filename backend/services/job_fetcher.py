...
JUNIOR_KEYWORDS = ["junior", "fresher", "entry", "associate", "graduate", "trainee"]
SENIOR_EXCLUDE = ["senior", "sr.", "lead", "principal", "director", "staff", "head of"]
...
async def fetch_jooble_jobs(skills: list, location: str = "India", job_titles: list = []) -> list:
    ...
    keywords = f"{primary_role} {skill_keywords} {' '.join(job_titles[:2]) if job_titles else 'junior fresher'}"
...
async def fetch_remotive_jobs(skills: list, job_titles: list = []) -> list:
    ...
    async with httpx.AsyncClient() as client:
        ...
        final_jobs = filtered if len(filtered) >= 1 else []
...
async def fetch_all_jobs(skills: list, location: str = "India", job_titles: list = []) -> list:
    ...
    is_fresher_search = any(kw in " ".join(job_titles).lower() for kw in JUNIOR_KEYWORDS)

    if is_fresher_search:
        return [job for job in unique_jobs if not any(word in job["title"].lower() for word in SENIOR_EXCLUDE)]
    return unique_jobs